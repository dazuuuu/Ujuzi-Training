...
return [
    'up' => static function (PDO $pdo): void {
        $pdo->prepare(
            ...
            'Hosts workplace attachment after a student finishes their course. Profile form should include attachment duration (start and end dates). Students only see attachment trainers once they have completed a course.',
        ]);
        ...
        if (!$exists) {
            $pdo->exec(
                "INSERT INTO roles (slug, name, description, has_admin_features, is_under_organisation, can_manage_users, managed_role_slugs, sort_order)
                 VALUES ('attachment_trainer', 'Attachment Trainer', 'Hosts workplace attachment after a student finishes their course. Profile form should include attachment duration (start and end dates).', 0, 1, 0, '[]', 20)"
            );
        }
    },
